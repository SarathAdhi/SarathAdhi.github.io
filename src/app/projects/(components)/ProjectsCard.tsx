...
const ProjectsCard: React.FC<Props> = ({
  ...
  return (
    <MotionLink
      href={`/projects/${id}`}
      className="relative w-full pd space-y-2"
      onHoverStart={() => {
    ...
    >
      <div className="relative flex items-center gap-10">
        <div className="space-y-2">
          ...
          <div className="ml-8 flex items-center flex-wrap gap-4">
            {techstack.map(({ name, image }) => (
              <ToolTip key={name} tooltip={name} side="bottom">
                <div className=" duration-200 hover:scale-125 w-10 h-10 bg-gray-100 dark:bg-white p-2 rounded-full">
                  <Image
